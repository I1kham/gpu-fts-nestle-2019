...
Core::Core()
{
    ...
    rs232BufferOUT = NULL;
    esapiModule.type = esapi::eExternalModuleType_unknown;
    esapiModule.verMajor = esapiModule.verMinor = 0;
    esapiModule.started = 0;
}
...
bool Core::open (const char *serialPort, const HThreadMsgW &hCPUServiceChannelW)
{
    ...
    rhea::thread::createMsgQ (&serviceMsgQR, &serviceMsgQW);
    {
        OSEvent h;
        rhea::thread::getMsgQEvent(serviceMsgQR, &h);
    ...
    esapi::ModuleRaw *m = RHEANEW(localAllocator, esapi::ModuleRaw)();
    if (!m->setup (localAllocator, logger, com, hCPUServiceChannelW, serviceMsgQR, serviceMsgQW))
    {
        RHEADELETE(localAllocator, m);
        return false;
	    logger->log ("FAIL\n");
	    logger->decIndent();
    }
    ...
    subscriberList.setup (localAllocator, 4);
...
void Core::priv_close ()
{
    ...
    if (localAllocator)
    {
        ...
        rhea::thread::getMsgQEvent(serviceMsgQR, &h);
        waitableGrp.removeEvent (h);
        rhea::thread::deleteMsgQ (serviceMsgQR, serviceMsgQW);
			...
			case OSWaitableGrp::evt_origin_osevent:
				{
                    ...
                    case WAITLIST_EVENT_FROM_A_SUBSCRIBER:
				        ...
				        {
					        if (rhea::event::compare(subscriberList(i2)->hEvent, waitableGrp.getEventSrcAsOSEvent(i)))
					        {
						        priv_handleMsgFromSubscriber(subscriberList.getElem(i2));
						        break;
					        }
				        }
                        break;
...
//***************************************************
Core::sSubscription* Core::priv_newSubscription()
{
	//creo le msgQ
...
//*********************************************************
void Core::priv_handleMsgFromServiceQ()
{
	rhea::thread::sMsg msg;
    while (rhea::thread::popMsg (serviceMsgQR, &msg))
    {
        ...
        case ESAPI_SERVICECH_SUBSCRIPTION_REQUEST:
			...
			{
				logger->log("ESAPI::Core => new SUBSCRIPTION_REQUEST\n");
				logger->incIndent();
...
//*********************************************************
void Core::priv_handleMsgFromSubscriber(sSubscription *sub)
{
    rhea::thread::sMsg msg;
    while (rhea::thread::popMsg (sub->q.hFromSubscriberToMeR, &msg))
    {
        ...
        {
        default:
            logger->log("ESAPI::Core() => invalid msg.what [0x%02X]\n", msg.what);
            break;
        ...
        case ESAPI_ASK_GET_MODULE_TYPE_AND_VER:
            notify_MODULE_TYPE_AND_VER (sub->q, handlerID, logger, esapiModule.type, esapiModule.verMajor, esapiModule.verMinor);
            break;

        case ESAPI_ASK_GET_IPandSSID:
            //questa richiesta è valida solo se dell'altra parte c'è un modulo ESAPI valido che sia ancora nella fase iniziale (started==0)
            if (esapiModule.type == esapi::eExternalModuleType_rasPI_wifi_REST && esapiModule.started == 0)
            {
                const u32 n = esapi::buildMsg_R0x05_getIPandSSID (rs232BufferOUT, SIZE_OF_RS232BUFFEROUT);
                this->priv_rs232_sendBuffer (com, rs232BufferOUT, n);
            }
            else
                esapi::notify_WIFI_IPandSSID (sub->q, handlerID, logger, "0.0.0.0", "err");
            break;

        case ESAPI_ASK_START_MODULE:
            //questa richiesta è valida solo se dell'altra parte c'è un modulo ESAPI valido che sia ancora nella fase iniziale (started==0)
            if (esapiModule.type == esapi::eExternalModuleType_rasPI_wifi_REST && esapiModule.started == 0)
            {
                const u32 n = esapi::buildMsg_R0x06_start (rs232BufferOUT, SIZE_OF_RS232BUFFEROUT);
                this->priv_rs232_sendBuffer (com, rs232BufferOUT, n);
                notify_MODULE_STARTED (sub->q, handlerID, logger);
            }
            else
            {
                DBGBREAK;
...
bool Core::priv_rs232_handleCommand_R (OSSerialPort &comPort, sBuffer &b)
{
	...
	case '1':
        ...
        //C'è un modulo esterno, collegato alla seriale che vuole identificarsi
		{
            //parse del messaggio
            bool bValidCk = false;
            const u32 MSG_LEN = esapi::buildMsg_R1_externalModuleIdentify_parseAsk (b.buffer, b.numBytesInBuffer, &bValidCk, &esapiModule.type, &esapiModule.verMajor, &esapiModule.verMinor);
            if (0 == MSG_LEN)
                return false;
            if (!bValidCk)
            {
