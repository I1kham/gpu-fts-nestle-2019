...
//*********************************************************
bool ModuleRasPI::priv_subscribeToCPUBridge(const HThreadMsgW &hCPUServiceChannelW)
{
    ...
    do
    {
        ...
        if (rhea::thread::popMsg(hTempMsgQR, &msg))
        {
            ...
            {
                glob->logger->log ("esapi::ModuleRasPI => subsribed to CPUBridge\n");
...
//*********************************************************
void ModuleRasPI::priv_handleMsgFromSubscriber(sSubscription *sub)
{
    rhea::thread::sMsg msg;
    while (rhea::thread::popMsg (sub->q.hFromSubscriberToMeR, &msg))
    {
        ...
        {
        default:
            glob->logger->log("ModuleRasPI::priv_handleMsgFromSubscriber() => invalid msg.what [0x%02X]\n", msg.what);
            break;
...
//*********************************************************
void ModuleRasPI::priv_onCPUNotify_RUNNING_SEL_STATUS(const rhea::thread::sMsg &msg)
{
    cpubridge::translateNotify_CPU_RUNNING_SEL_STATUS(msg, &runningSel.status);
}

//*********************************************************
void ModuleRasPI::priv_handleIncomingMsgFromCPUBridge()
{
	rhea::thread::sMsg msg;
...
//*********************************************************
void ModuleRasPI::priv_rs232_handleCommunication (sBuffer &b)
{
    while (1)
    {
 ...
 */
bool ModuleRasPI::priv_rs232_handleCommand_A (sBuffer &b)
{
 ...
 */
bool ModuleRasPI::priv_rs232_handleCommand_C (sBuffer &b)
{
 ...
 * funziona come priv_mode_raw_handleCommand_A(), vedi i commenti in cima a quella fn
 */
bool ModuleRasPI::priv_rs232_handleCommand_S (sBuffer &b)
{
 ...
 * funziona come priv_mode_raw_handleCommand_A(), vedi i commenti in cima a quella fn
 */
bool ModuleRasPI::priv_rs232_handleCommand_R (sBuffer &b)
{
