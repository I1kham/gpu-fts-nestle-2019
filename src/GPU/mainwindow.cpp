...
#include "../rheaGUIBridge/CmdHandler/CmdHandler_eventReqSelPrices.h"
#include "../rheaGUIBridge/CmdHandler/CmdHandler_eventReqSelAvailability.h"
...
{
    guibridge::CmdHandler_selAvailability_buildAResponseAndPushItToServer (hQMessageToWebserver, guibridge::eEventType_selectionAvailabilityUpdated, numSel, selAvailList.getBitSequence());
}
void MainWindow::guibridgeEvent_selectionPricesUpdated (int numSel, const unsigned int *priceList) const
{
    guibridge::CmdHandler_selPrices_buildAResponseAndPushItToServer (hQMessageToWebserver, guibridge::eEventType_selectionPricesUpdated, numSel, priceList);
}
void MainWindow::guibridgeEvent_selectionReqStatus (eSelectionReqStatus status) const
{
    guibridge::CmdHandler_selStatus_buildAResponseAndPushItToServer (hQMessageToWebserver, (u8)status);
}
...
void MainWindow::priv_handleGUIBridgeServerMessages()
{
    ...
    if ((msg.what & 0xFF00) == 0x0100)
    {
        ...
        case GUIBRIDGE_REQ_SELAVAILABILITY:
            guibridge::CmdHandler_selAvailability_buildAResponseAndPushItToServer (hQMessageToWebserver, handlerID, NUM_MAX_SELECTIONS, selAvailability.getBitSequence());
            break;

        case GUIBRIDGE_REQ_SELPRICES:
            guibridge::CmdHandler_selPrices_buildAResponseAndPushItToServer (hQMessageToWebserver, handlerID, NUM_MAX_SELECTIONS, Prices);
            break;
    ...
    if(VMCstate != VMCstate_old)
    {
        ...
        case VMCSTATE_PROGRAMMAZIONE:
         //   QApplication::setOverrideCursor(Qt::PointingHandCursor);
            ...
            myFormProg.initForm(lang_getCurLanguage(&language));
            myFormProg.show();
            break;
