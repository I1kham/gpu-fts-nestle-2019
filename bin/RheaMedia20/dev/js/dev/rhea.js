...
Rhea_clearSessionData = function (defaultLanguage)
{
	...
	Rhea_session_setValue("isFreevend", "0");
	...
	Rhea_session_setValue("isRFIDJugCard", "0");
	Rhea_session_setValue("isRFIDDebtCard", "0");
	...
	Rhea_session_setValue ("sPIQRCode", "");
}
...
function Rhea()
{
	...
	//Variabile di sessione: isFreevend
	//Viene gestita in autonomia, in modo che this.isFreevend rifletta sempre l'attuale stato di CPU
	this.isFreevend = 0;
	if (Rhea_session_getOrDefault("isFreevend", "0") == "1")
		this.isFreevend = 1;
	
	//Variabile di sessione: isTestvend
	//Viene gestita in autonomia, in modo che this.isTestvend rifletta sempre l'attuale stato di CPU
	this.isTestvend = 0;
	if (Rhea_session_getOrDefault("isTestvend", "0") == "1")
		this.isTestvend = 1;
	
	
	//Variabile di sessione: isRFIDJugCard
	//Viene gestita in autonomia, in modo che this.isRFIDJugCard rifletta sempre l'attuale stato di CPU
	this.isRFIDJugCard = 0;
	if (Rhea_session_getOrDefault("isRFIDJugCard", "0") == "1")
		this.isRFIDJugCard = 1;

	//Variabile di sessione: isRFIDDebtCard
	//Viene gestita in autonomia, in modo che this.isRFIDDebtCard rifletta sempre l'attuale stato di CPU
	this.isRFIDDebtCard = 0;
	if (Rhea_session_getOrDefault("isRFIDDebtCard", "0") == "1")
		this.isRFIDDebtCard = 1;
		...
		if (data.length >= 5)
		{
			if (data[0] == 35 && data[1] == 101 && data[2] == 86 && data[3] == 110)
			{
				...
				case RHEA_EVENT_CPU_STATUS:
					...
					if ( (flag16 & 0x08) == 0)	{ Rhea_session_setValue("isFreevend", "0"); me.isFreevend = 0; }
					else						{ Rhea_session_setValue("isFreevend", "1"); me.isFreevend = 1; }
					
					if ( (flag16 & 0x10) == 0)	{ Rhea_session_setValue("isTestvend", "0"); me.isTestvend = 0; }
					else						{ Rhea_session_setValue("isTestvend", "1"); me.isTestvend = 1; }

					if ( (flag16 & 0x20) == 0)	{ Rhea_session_setValue("isRFIDDebtCard", "0"); me.isRFIDDebtCard = 0; }
					else						{ Rhea_session_setValue("isRFIDDebtCard", "1"); me.isRFIDDebtCard = 1; }

					if ( (flag16 & 0x40) == 0)	{ Rhea_session_setValue("isRFIDJugCard", "0"); me.isRFIDJugCard = 0; }
					else						{ Rhea_session_setValue("isRFIDJugCard", "1"); me.isRFIDJugCard = 1; }
			...
			if (data[0] == 35 && data[1] == 102 && data[2] == 84 && data[3] == 114)
			{
				...
				switch (opcode)
				{
					case 2:
						...
						{
							if (null!=me.fileTransfer[i].appTransfUID && me.fileTransfer[i].appTransfUID == sData0x02.appTransfUID)
							{
					...
					case 4:
						...
						for (var i=0; i<me.nFileTransfer; i++)
						{
							if (me.fileTransfer[i] != null && me.fileTransfer[i].appTransfUID == sData0x04.appTransfUID)
							{
	...
	return new Promise( function(resolve, reject) 
					{
						...
						var check = function()	{
												...
												else if ((timeoutMs -= CHECK_TIME_MS) < 0)
												{
...
Rhea.prototype.ajaxWithCustomTimeout = function(commandString, plainJSObject, timeoutMSec)
{
	...
	//le stringhe in js sono in UCS2 o UTF16, io voglio inviare i dati in UTF8
	var joUTF8 = jsStringToUTF8Array(jo);

	var bytesNeeded = 	1 							//lenght di [commandString]
						+ commandString.length		//[commandString]
						+ 2							//lenght di [plainJSObject]
						+ joUTF8.length				//[plainJSObject]
	...
	var buffer = new Uint8Array(bytesNeeded);
		...
		buffer[t++] = ((joUTF8.length & 0xFF00) >> 8);
		buffer[t++] = (joUTF8.length & 0x00FF);
		for (var i=0; i<joUTF8.length; i++)
			buffer[t++] = joUTF8[i];
