...
console.time("page general menu");		
	pleaseWait_addMessage("creating page menu files...");
	await buildConfigFile_pageMenuGen(db, historyID);
console.timeEnd("page general menu");	
...
console.time("main menu menu icons");	
	pleaseWait_addMessage("creating main menu icons files...");
	await buildConfigFile_mainMenuIcons_gen(db, historyID);
console.timeEnd("main menu menu icons");		
...
console.time("  page standby[rst3]");
    rst = await db.q("SELECT * FROM pageMenugen_gen WHERE HIS_ID=" +his_id);
	result +=",menuEnabled:"+"\"" +rst.valByColName(0, "menu_enabled") +"\"";
console.timeEnd("  page standby[rst3]");
	result += "};";		
 ...
 * crea il file config/pageMenu.js
 */
async function buildConfigFile_pageMenuGen(db, his_id)
{
	let rst = await db.q("SELECT * FROM pagemenu_menugen_gen WHERE HIS_ID=" +his_id);
	var result = "var mainMenuGenPageOptions={"
				+"menu_numRow:" +rst.valByColName(0, "icon_numRow")
				+",menu_numIconPerRow:" +rst.valByColName(0, "icon_numIconPerRow")
				+",menu_gotoPageStandbyAfterMSec:" +rst.valByColName(0, "gotoPageStandbyAfterMSec")
				+",menu_oneClickSelection:" +rst.valByColName(0, "oneClickSelection")
				+",menu_enabled:" +rst.valByColName(0, "menu_enabled")
				+"};";
	
	await buildConfigFile_saveAs (result, rheaGetAbsolutePhysicalPath()+"/../config", "pageMenuGen.js");
}
...
/*********************************************************
 * crea il file config/mainMenuIconsGen.js per i menu generali 
 */
async function buildConfigFile_mainMenuIcons_gen(db, his_id)       //20211105
{
console.time("  main menu icons gen [rst]");
	let rst = await db.q("SELECT * FROM pagemenu_menugen WHERE HIS_ID=" +his_id +" ORDER BY PROGR");
console.timeEnd("  main menu icons gen [rst]");	

	var result = "var rheaMainMenuIconsGen = [";
	for (var r=0; r<rst.getNumRows(); r++)
		result += "{" +priv_buildConfigFile_mainMenuIconsGen_2 (rst, r) +"},\n";
	result +="];";

console.time("  main menu icons gen [saveas]");	
	await buildConfigFile_saveAs (result, rheaGetAbsolutePhysicalPath()+"/../config", "mainMenuIconsGen.js");
console.timeEnd("  main menu icons gen [saveas]");		
}
...
function priv_buildConfigFile_mainMenuIconsGen_2 (rst, row)
{
	if (rst.valByColName(row, "menuGenNum") > 0)
	{
		//selezione senza cup customization
...
console.time("translation 01");
	...
	rstMenuSel = await db.q("SELECT UID,menuGenNum,name,secondName FROM pagemenu_menugen WHERE HIS_ID=" +his_id +" ORDER BY PROGR");
...
console.time("translation per ogni lingua");
	var defaultSelName=[];
	var defaultSelInfo=[];
	var defaultMenuSelName=[];
	var defaultMenuSelDescr=[];
...
console.timeEnd("  " +iso +"06");	
		...
		//nomi dei menu per lingua------------------------------------
console.time("  " +iso +"05M");
console.time("  " +iso +"05M_rst");
		let rstTranslationM = await db.q("SELECT What,Message FROM lang WHERE UID='MGE_NAME' AND ISO='" +iso +"'");
console.timeEnd("  " +iso +"05M_rst");		
		var menuNames = "";
		for (var iMenuSel=0; iMenuSel<rstMenuSel.getNumRows(); iMenuSel++)
		{
			if (menuNames!="")
				menuNames +=",";
			
			var UID = rstMenuSel.valByColName(iMenuSel, "UID");
			var s = priv_buildConfigFile_translation_findWHAT (rstTranslationM, UID);
			
			//se vuoto, default alla traduzione del linguaggio di default
			if (iLang==0)
				defaultMenuSelName[iMenuSel] = s;
			else if (s=="" || s=="NULL")
				s = defaultMenuSelName[iMenuSel];
			
			menuNames +="\"" +s +"\"";
		}
		menuNames = "var rheaLang_mainMenuMenuName=[" +menuNames +"];";
		result += menuNames;
		// Retrieves second name for beverage
		let rstsecondMenuNameDescription = await db.q("SELECT secondName FROM pagemenu_menugen ORDER BY PROGR ASC");
		var secondMenuName = '';

		for( var i=0; i<rstsecondMenuNameDescription.getNumRows(); i++ ) {
			var _what = rstsecondMenuNameDescription.valByColName(i, 'secondName');

			if( secondMenuName ) { secondMenuName += ','; }

			secondMenuName +="\"" + _what +"\"";
		}


/*		let rstTranslationMD1 = await db.q("SELECT What,Message FROM lang WHERE UID='MGE_DESCR' AND ISO='" +iso +"'");
		...
		for (var iMenuSel=0; iMenuSel<rstMenuSel.getNumRows(); iMenuSel++)
		{
			...
			var UID = rstMenuSel.valByColName(iMenuSel, "UID");
			var s1 = priv_buildConfigFile_translation_findWHAT (rstTranslationMD1, UID);
			
			//se vuoto, default alla traduzione del linguaggio di default
			if (iLang==0)
				defaultMenuSelDescr[iMenuSel] = s1;
			else if (s1=="" || s1=="NULL")
				s1 = defaultMenuSelDescr[iMenuSel];
...
*/
		...
		secondMenuName = "var rhea_mainMenuMenuSecondName=[" + secondMenuName +"];";
		result += secondMenuName;
		
		
console.time("  " +iso +"05M_saveas");
		await buildConfigFile_saveAs (result, rheaGetAbsolutePhysicalPath()+"/../config/lang", iso +".js");
console.timeEnd("  " +iso +"05M_saveas");
console.timeEnd("  " +iso +"05M");
		//fine nomi dei menu per lingua-------------------------------
